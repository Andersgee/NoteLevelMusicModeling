...
function ∇grid!(C,N,d, bn,∇WHib,∇hi,∇ho,ho,g,mi,mo,∇W,Σ∇b,Hi,Σ∇W, ∇Hi,Σ∇Hi,W,∇mi,∇mo)
    ...
    for c=C:-1:1
        ...
        for n=1:N 
            ...
            for gate=1:4
                A_mul_Bt!(∇W[n][gate], ∇WHib[gate], Hi[c]) #∇W[gate] = ∇WHib[gate] * Hi[c]'
                Σ∇W[n][gate] .+= ∇W[n][gate]
                Σ∇b[n][gate] .+= ∇WHib[gate]
                At_mul_B!(∇Hi[gate], W[n][gate], ∇WHib[gate]) #∇Hi[gate] = W[gate]' * ∇WHib[gate]
                Σ∇Hi .+= ∇Hi[gate]
            end
        end
...
function gridvars(N,C,d,bsz)
  ...
  for n=1:N
    fill!(b[n][2], 1.0) #positive bias to gate gf=σ(W*x+b) will backprob better since: ∇mi = ∇mo.*gf
    #with bias 1; gf will start with a mean 0.73 instead of 0.5
  end
